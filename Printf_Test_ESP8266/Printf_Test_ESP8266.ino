...
void setup() {
  ...
  Serial.println();
  #endif // ESP8266
  Serial.printf_P(PSTR("Starting Printf_Test version %d.%d, Build date %s %s\n"), VER_MAJOR,
                                                                                  VER_MINOR,
                                                                                  __DATE__,
                                                                                  __TIME__);
  ...
  rtn = printfKGW.printf_P(PSTR("printfKGW PGM string strlen 31\n"));
  Serial.printf("printfKGW PGM rtn = %d\n", rtn);
  #endif // BLOMP_KGW
  
  Serial.printf(  "printf RAM: Verison %d.%d\n",  VER_MAJOR, VER_MINOR);
  Serial.printf_P(PSTR("printf PGM: Verison %d.%d\n"), VER_MAJOR, VER_MINOR);
  ...
  Serial.printf_P(PSTR("Ending setup. millis() = %lu\n"), millis());
}
